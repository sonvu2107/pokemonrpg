...
export const battleTrainerApi = {
    ...
    async deleteAutoGenerated(params = {}) {
        const searchParams = new URLSearchParams()
        if (params?.fromLevel !== undefined && params?.fromLevel !== null && params?.fromLevel !== '') {
            searchParams.set('fromLevel', String(params.fromLevel))
        }
        if (params?.toLevel !== undefined && params?.toLevel !== null && params?.toLevel !== '') {
            searchParams.set('toLevel', String(params.toLevel))
        }
        const query = searchParams.toString()

        const res = await fetch(`${API_URL}/admin/battle-trainers/auto-generated${query ? `?${query}` : ''}`, {
            method: 'DELETE',
            headers: getAuthHeader(),
        })
        if (!res.ok) {
