...
export function BattlePage() {
    ...
    const loadMaps = async () => {
        try {
            const allMaps = await mapApi.list()
            setMaps(allMaps)
        } catch (error) {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {maps.map(map => (
                                <Link
                                    key={map._id}
                                    to={`/map/${map.slug}`}
                                    className="group block bg-white rounded-lg border border-slate-200 overflow-hidden shadow-sm hover:shadow-md hover:border-blue-400 transition-all duration-200"
                                >
                                    <div className="px-3 py-2 bg-gradient-to-r from-slate-100 to-white border-b border-slate-100 flex justify-between items-center group-hover:from-blue-50 group-hover:to-white transition-colors">
                                        ...
                                        {map.isLegendary && <span className="text-[10px] uppercase font-bold text-amber-600 bg-amber-100 px-1.5 py-0.5 rounded">Huyền Thoại</span>}
                                    </div>
                                    <div className="p-3">
