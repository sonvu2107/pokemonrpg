...
const getSprite = (listing) => {
    if (!listing) return ''
    return resolveImageSrc(listing.sprite || 'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/0.png')
}
        ...
        <div className="max-w-4xl mx-auto pb-12 font-sans">
            ...
            {marketTab === 'pokemon' ? <div className="space-y-4">
                ...
                <section className="border border-blue-400 rounded-t-lg overflow-hidden shadow-sm bg-white">
                    ...
                    <div className="overflow-x-auto"><table className="w-full min-w-[500px]"><thead><tr className="bg-blue-50 border-b border-blue-300 text-blue-900 text-[11px] sm:text-sm font-bold"><th className="px-2 py-2 text-left w-36 sm:w-auto">Vật phẩm</th><th className="px-1 py-2 text-center w-16 sm:w-auto">Công dụng</th><th className="px-1 py-2 text-center w-8 sm:w-auto">SL</th><th className="px-1 py-2 text-center w-24 sm:w-auto">Giá</th><th className="px-1 py-2 text-center w-20 sm:w-auto">{itemFilters.display === 'bought_by_you' ? 'Người bán' : (itemFilters.display === 'sold_by_you' ? 'Người mua' : 'Người bán')}</th><th className="px-1 py-2 text-center w-16 sm:w-auto">{itemFilters.display === 'all' ? 'Mua' : 'Trạng thái'}</th></tr></thead><tbody>{loading ? <tr><td colSpan={6} className="px-3 py-8 text-center text-slate-500 font-bold">Đang tải chợ vật phẩm...</td></tr> : itemListings.length === 0 ? <tr><td colSpan={6} className="px-3 py-8 text-center text-slate-500">Chưa có lịch sử/phiên giao dịch vật phẩm phù hợp.</td></tr> : itemListings.map((listing) => <tr key={listing.id} className="border-b border-blue-100 hover:bg-blue-50/40"><td className="px-2 py-2 sm:px-3 sm:py-3"><div className="flex items-center gap-1 sm:gap-3"><img src={resolveImageSrc(listing.itemImageUrl || 'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/poke-ball.png')} alt={listing.itemName} className="w-8 h-8 sm:w-10 sm:h-10 object-contain shrink-0" /><div className="min-w-0"><div className="font-bold text-slate-800 text-xs sm:text-sm truncate">{listing.itemName}</div><div className="text-[10px] sm:text-xs text-slate-500 truncate">{ITEM_TYPE_LABELS[listing.itemType] || listing.itemType}</div></div></div></td><td className="px-1 py-2 sm:px-3 sm:py-3 text-center text-[10px] sm:text-sm font-semibold text-slate-700">{ITEM_UTILITY_LABELS[listing.effectCategory] || listing.effectCategoryLabel || listing.effectCategory}</td><td className="px-1 py-2 sm:px-3 sm:py-3 text-center font-bold text-xs sm:text-sm">x{listing.quantity}</td><td className="px-1 py-2 sm:px-3 sm:py-3 text-center font-bold text-xs sm:text-sm">{Number(listing.price || 0).toLocaleString('vi-VN')} xu</td><td className="px-1 py-2 sm:px-3 sm:py-3 text-center font-bold text-slate-700 text-[10px] sm:text-sm break-words">{itemFilters.display === 'sold_by_you' ? (listing.buyer?.username || 'Không rõ') : (listing.seller?.username || 'Không rõ')}</td><td className="px-1 py-2 sm:px-3 sm:py-3 text-center">{itemFilters.display === 'all' ? <button type="button" onClick={() => handleBuyItem(listing)} disabled={buyingId === listing.id} className="px-2 py-1 sm:px-3 sm:py-1.5 bg-white border border-blue-300 text-blue-700 font-bold text-[10px] sm:text-xs hover:bg-blue-50 disabled:opacity-50 break-keep shrink-0">{buyingId === listing.id ? 'Mua...' : 'Mua'}</button> : <span className="inline-block rounded border border-emerald-200 bg-emerald-50 px-1 py-0.5 sm:px-2 sm:py-1 text-[9px] sm:text-xs font-bold text-emerald-700 leading-tight">Đã GD</span>}</td></tr>)}</tbody></table></div>
                </section>
            </div>}
