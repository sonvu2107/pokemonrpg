...
export default function BattleTrainerPage() {
    ...
    const [showDeleteAutoGeneratedModal, setShowDeleteAutoGeneratedModal] = useState(false)
    const [deleteAutoGeneratedFromLevel, setDeleteAutoGeneratedFromLevel] = useState('')
    const [deleteAutoGeneratedToLevel, setDeleteAutoGeneratedToLevel] = useState('')
    const [deleteAutoGeneratedModalError, setDeleteAutoGeneratedModalError] = useState('')
    ...
    const [trainerPokemonSearchInput, setTrainerPokemonSearchInput] = useState('')
    const [trainerPokemonSearchQuery, setTrainerPokemonSearchQuery] = useState('')
    ...
    const loadTrainers = async (targetPage = trainerPagination.page, searchQuery = trainerPokemonSearchQuery) => {
        try {
            setLoading(true)
            setError('')
            ...
            const [data, usageSummaryData] = await Promise.all([
                battleTrainerApi.list({
                    ...
                    ...(normalizedSearchQuery ? { searchTrainer: normalizedSearchQuery } : {}),
                }),
    ...
    const handleApplyTrainerPokemonSearch = async () => {
        const normalizedSearchQuery = String(trainerPokemonSearchInput || '').trim()
        setTrainerPokemonSearchQuery(normalizedSearchQuery)
        setTrainerPagination((prev) => ({ ...prev, page: 1 }))
        if (trainerPagination.page === 1) {
            await loadTrainers(1, normalizedSearchQuery)
        }
    }

    const handleClearTrainerPokemonSearch = async () => {
        setTrainerPokemonSearchInput('')
        setTrainerPokemonSearchQuery('')
        setTrainerPagination((prev) => ({ ...prev, page: 1 }))
        if (trainerPagination.page === 1) {
    ...
    const handleDeleteAutoGenerated = async () => {
        if (autoGeneratedTrainerCount === 0) return

        const parsedFromLevel = Number.parseInt(deleteAutoGeneratedFromLevel, 10)
        const parsedToLevel = Number.parseInt(deleteAutoGeneratedToLevel, 10)
        ...
        if (fromLevel !== null && toLevel !== null && fromLevel > toLevel) {
            setDeleteAutoGeneratedModalError('Mốc từ level phải nhỏ hơn hoặc bằng mốc đến level.')
            return
        }
        ...
        const rangeText = fromLevel !== null || toLevel !== null
            ...
            : ''
        if (!confirm(`Xóa trainer auto-generated${rangeText}? Trainer tạo tay sẽ được giữ lại.`)) return

        try {
            setError('')
            setDeleteAutoGeneratedModalError('')
            setDeletingAutoGenerated(true)
            await battleTrainerApi.deleteAutoGenerated({ fromLevel, toLevel })
            resetForm()
            setShowDeleteAutoGeneratedModal(false)
            setDeleteAutoGeneratedFromLevel('')
            setDeleteAutoGeneratedToLevel('')
            setTrainerPagination((prev) => ({ ...prev, page: 1 }))
            await loadTrainers(1)
        } catch (err) {
            const resolvedError = err.message || 'Xóa trainer auto-generated thất bại'
            setDeleteAutoGeneratedModalError(resolvedError)
    ...
    const handleOpenDeleteAutoGeneratedModal = () => {
        if (autoGeneratedTrainerCount === 0) return
        setDeleteAutoGeneratedFromLevel('')
        setDeleteAutoGeneratedToLevel('')
        setDeleteAutoGeneratedModalError('')
        setShowDeleteAutoGeneratedModal(true)
    }
    ...
    return (
        <div className="rounded border border-blue-400 bg-white shadow-sm">
                    ...
                    <div className="mb-2 flex items-center justify-between gap-2 flex-wrap">
                        <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                                <input
                                    type="text"
                                    value={trainerPokemonSearchInput}
                                    onChange={(e) => setTrainerPokemonSearchInput(e.target.value)}
                                    onKeyDown={(e) => {
                                        if (e.key === 'Enter') {
                                            e.preventDefault()
                                            handleApplyTrainerPokemonSearch()
                                        }
                                    }}
                                    placeholder="Tìm theo tên trainer để nhảy trang"
                                    className="w-72 max-w-full px-3 py-1.5 bg-white border border-slate-300 rounded text-xs text-slate-700"
                                />
                                <button
                                    type="button"
                                    onClick={handleApplyTrainerPokemonSearch}
                                    disabled={loading}
                                    className="px-2.5 py-1.5 text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white rounded disabled:opacity-50"
                                >
                                    Tìm trainer
                                </button>
                                {trainerPokemonSearchQuery && (
                                    <button
                                        type="button"
                                        onClick={handleClearTrainerPokemonSearch}
                                        disabled={loading}
                                        className="px-2.5 py-1.5 text-xs font-bold bg-white border border-slate-300 text-slate-700 rounded disabled:opacity-50"
                                    >
                                        Xóa lọc
                                    </button>
                                )}
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                type="button"
                                onClick={handleOpenDeleteAutoGeneratedModal}
            ...
            {showDeleteAutoGeneratedModal && (
                <div
                    className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[72] p-4 animate-fadeIn"
                    onClick={() => {
                        if (deletingAutoGenerated) return
                        setShowDeleteAutoGeneratedModal(false)
                    }}
                >
                    <div
                        className="bg-white rounded-lg border border-slate-200 p-4 sm:p-6 w-full max-w-lg shadow-2xl"
                        onClick={(e) => e.stopPropagation()}
                    >
                        <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-3">
                            <div>
                                <h3 className="text-lg font-bold text-slate-800">Xóa trainer auto theo mốc level</h3>
                                <p className="text-xs text-slate-500 mt-0.5">Chỉ xóa trainer auto-generated, trainer tạo tay sẽ được giữ nguyên.</p>
                            </div>
                            <button
                                type="button"
                                onClick={() => setShowDeleteAutoGeneratedModal(false)}
                        ...
                        <div className="space-y-3">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label className="block text-slate-700 text-sm font-bold mb-1.5">Từ mốc level</label>
                                    <input
                                        type="number"
                                        min="0"
                                        value={deleteAutoGeneratedFromLevel}
                                        onChange={(e) => setDeleteAutoGeneratedFromLevel(e.target.value)}
                                        placeholder="Để trống = không giới hạn"
                                        className="w-full px-3 py-2 bg-white border border-slate-300 rounded-md text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent shadow-sm"
                                    />
                                </div>
                                <div>
                                    <label className="block text-slate-700 text-sm font-bold mb-1.5">Đến mốc level</label>
                                    <input
                                        type="number"
                                        min="0"
                                        value={deleteAutoGeneratedToLevel}
                                        onChange={(e) => setDeleteAutoGeneratedToLevel(e.target.value)}
                                        placeholder="Để trống = không giới hạn"
                                        className="w-full px-3 py-2 bg-white border border-slate-300 rounded-md text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent shadow-sm"
                                    />
                                </div>
                            ...
                            <div className="text-xs text-slate-500 rounded border border-slate-200 bg-slate-50 px-3 py-2">
                                Đang có {autoGeneratedTrainerCount} trainer auto-generated trong hệ thống. Bạn có thể nhập một đầu mốc hoặc cả hai đầu mốc để xóa theo khoảng.
                            </div>

                            {deleteAutoGeneratedModalError && (
                            ...
                            <div className="flex items-center justify-end gap-2 pt-1">
                                <button
                                    ...
                                    onClick={() => setShowDeleteAutoGeneratedModal(false)}
                                    disabled={deletingAutoGenerated}
                                    className="px-3 py-2 text-xs font-bold border border-slate-300 bg-white text-slate-700 rounded hover:bg-slate-50 disabled:opacity-50"
                                >
                                    Hủy
                                </button>
                                <button
                                    ...
                                    disabled={deletingAutoGenerated}
                                    className="px-3 py-2 text-xs font-bold bg-amber-600 hover:bg-amber-700 text-white rounded disabled:opacity-50"
                                >
                                    {deletingAutoGenerated ? 'Đang xóa auto...' : 'Xóa trainer auto'}
                                </button>
                            </div>
                        </div>
                    </div>
