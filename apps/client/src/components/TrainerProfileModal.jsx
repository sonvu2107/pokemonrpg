...
export default function TrainerProfileModal({
    ...
    const equippedBadges = Array.isArray(trainer?.badges?.equippedBadges)
        ? trainer.badges.equippedBadges
        : []
    const activeBadgeBonuses = trainer?.badges?.activeBonuses || {}
    ...
    return (
        ...
        >
            <div className="border border-blue-400 rounded overflow-hidden shadow-sm">
                <ProfileSectionHeader title={<><span>Hồ sơ của </span><VipUsername userLike={trainer}>{trainer.username || 'Huấn Luyện Viên'}</VipUsername></>} />
