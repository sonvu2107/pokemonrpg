...
import { hasOwnedPokemonForm } from './userPokemonOwnershipService.js'

const clamp = (value, min, max) => Math.max(min, Math.min(max, value))

const createServiceError = (message, { status = 400, code = '' } = {}) => {
...
export const searchMapForUserDirect = async ({ userId, mapSlug, role = 'user', vipTierLevel = 0 } = {}) => {
    const normalizedUserId = String(userId || '').trim()
    const normalizedMapSlug = String(mapSlug || '').trim().toLowerCase()
    const normalizedRole = String(role || '').trim().toLowerCase()
    const isAdmin = normalizedRole === 'admin'
    const currentVipLevel = Math.max(0, Number(vipTierLevel) || 0)

    if (!normalizedUserId || !normalizedMapSlug) {
        throw createServiceError('Thiếu userId hoặc mapSlug', { status: 400, code: 'INVALID_INPUT' })
    }

    const map = await MapModel.findOne({ slug: normalizedMapSlug })
    if (!map) {
        throw createServiceError('Không tìm thấy bản đồ', { status: 404, code: 'MAP_NOT_FOUND' })
    }

    const playerLevelState = await PlayerState.findOne({ userId: normalizedUserId })
        .select('level')
        .lean()
    const currentPlayerLevel = Math.max(1, Number(playerLevelState?.level) || 1)

    const orderedMaps = await getOrderedMapsCached()
    const mapIndex = orderedMaps.findIndex((entry) => entry._id.toString() === map._id.toString())
    if (mapIndex === -1) {
        throw createServiceError('Không tìm thấy bản đồ trong thứ tự tiến trình', {
            status: 404,
            code: 'MAP_ORDER_NOT_FOUND',
        })
    }
    ...
    if (!isAdmin) {
        ...
        if (sourceMap?._id) {
            const sourceProgress = await MapProgress.findOne({ userId: normalizedUserId, mapId: sourceMap._id })
        ...
        if (!isUnlocked) {
            const error = createServiceError('Bản đồ chưa mở khóa', { status: 403, code: 'MAP_LOCKED' })
            error.locked = true
            error.unlock = unlockRequirement
            throw error
        }
    }

    const mapProgress = await updateMapProgress(normalizedUserId, map._id)
    const { playerState, leveledUp, levelsGained } = await updatePlayerLevel(normalizedUserId)
    ...
    if (!isAdmin) {
        ...
        if (nextMap?._id) {
            ...
            if (mapProgress.totalSearches >= requiredToUnlockNext) {
                await ensureMapUnlocked(normalizedUserId, nextMap._id)
            }
        }
    }
    ...
    if (shouldDropItem) {
        ...
        if (selectedItemDrop?.itemId) {
            ...
            await UserInventory.findOneAndUpdate(
                { userId: normalizedUserId, itemId: storedItem._id },
                { $inc: { quantity: 1 } },
    ...
    const hp = maxHp
    const playerBattleSnapshot = await resolveWildPlayerBattleSnapshot(normalizedUserId)
    const isNewPokedexEntry = !(await hasOwnedPokemonForm(normalizedUserId, pokemon._id, formId))
