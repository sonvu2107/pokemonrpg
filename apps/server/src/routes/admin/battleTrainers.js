...
router.post('/', async (req, res) => {
    try {
        ...
        const trainer = new BattleTrainer({
            ...
            moonPointsReward: parseRewardNumber(moonPointsReward, 0),
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (moonPointsReward !== undefined) trainer.moonPointsReward = parseRewardNumber(moonPointsReward, 0)
        if (autoGenerated !== undefined) trainer.autoGenerated = Boolean(autoGenerated)
...
router.post('/auto-generate', async (req, res) => {
    try {
        ...
        const configuredMoonPointsReward = parseOptionalRewardNumber(req.body?.moonPointsReward)
        ...
        const upsertResults = await Promise.all(
            levels.map(async (level, index) => {
                ...
                const moonPointsReward = configuredMoonPointsReward !== null ? configuredMoonPointsReward : rewardValue
