...
router.get('/', async (req, res) => {
    try {
        ...
        const normalizedSearchTrainer = String(req.query.searchTrainer || '').trim()

        let trainerFilter = {}
        if (normalizedSearchTrainer) {
            const escapedSearchTrainer = escapeRegex(normalizedSearchTrainer)
            trainerFilter = {
                name: { $regex: escapedSearchTrainer, $options: 'i' },
            }
        }

        const [trainers, total, autoGeneratedTotal] = await Promise.all([
            BattleTrainer.find(trainerFilter)
                ...
                .lean(),
            BattleTrainer.countDocuments(trainerFilter),
