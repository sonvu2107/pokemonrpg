...
const buildAutoTeam = (pokemonPool, level, seed, teamSize = 3) => {
    ...
    for (let slot = 0; slot < normalizedTeamSize; slot += 1) {
        ...
        const adjustedLevel = clampNumber(level, 1, 100)
...
router.post('/auto-generate', async (req, res) => {
    try {
        const startLevel = clampNumber(req.body?.startLevel, 1, 100)
        const maxLevel = clampNumber(req.body?.maxLevel, startLevel, 100)
        const step = clampNumber(req.body?.step, 1, 100)
                ...
                const trainerDoc = await BattleTrainer.findOneAndUpdate(
                    ...
                    },
                    {
                        $set: {
                            ...
                            prizeItemId: null,
                            prizeItemQuantity: 1,
                            platinumCoinsReward: rewardValue,
                            expReward: rewardValue,
                            moonPointsReward: rewardValue,
                            autoGenerated: true,
                            milestoneLevel: level,
                        },
                    },
